...
Endpoints (무로그인 개방, 조회 전용):
    ...
    GET /dashboard/attention 지금 조치 필요(점도 미입력·자재 LOT 파일 경과·오늘 실적)
    GET /dashboard/recent    최근 배합 기록 (점도 포함)
...
def build_router() -> APIRouter:
    ...
    def dashboard_summary(
        ...
        with get_connection() as connection:
            ...
            # 현재 상태 스냅샷(기간과 무관한 '지금 해야 할 일')은 /attention 이 낸다.
    ...
    def dashboard_attention() -> dict[str, Any]:
        ...
        # NOTE: unacked_count(rescale/manual 미확인 건수)와 today(오늘 날짜)는 2026-08-14
        # 제거했다 — 미확인 건수는 무로그인 노출되는 값인데 정작 화면에선 /api/blend/*
        # unacked 를 따로 부르므로 쓰이지 않았고, today 는 어느 곳에서도 읽지 않는 죽은 필드.
        return {
    ...
    def dashboard_recent(
        ...
        # COALESCE(is_bulk_regenerated,0)=0 — summary/trend/attention 과 같은 기준.
        # 일괄 재생성분은 실배합이 아니라 최근 기록에만 섞여 들어와 집계를 어긋나게 했다.
        # approved(결재 여부)는 결재 기능이 2026-07 제거됨에 따라 계산·반환하지 않는다.
        with get_connection() as connection:
            rows = connection.execute(
                """
