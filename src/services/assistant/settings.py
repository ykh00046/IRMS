...
# 2026-09-10 계정 실측(성공 응답의 x-ratelimit-* 헤더. Groq 는 429 없이도 알려 준다):
#   셋 다 하루 1,000회 · 분당 8,000토큰이고 한도는 **모델별**이라 사슬 전체로 하루 3,000회.
#   Gemini 무료(분당 5회)보다 넉넉해 뺄 이유가 없다. 다만 분당 8,000토큰이 진짜 천장이다 —
#   한 답변에 계획+작성 두 번 왕복하며 3,000~6,000토큰을 쓰므로 분당 1~2건이면 걸린다.
#   후보에서 뺀 것: groq/compound 계열은 하루 250회로 적고, qwen3.6-27b 는 <think> 를
#   그대로 뱉으며, allam-2-7b 는 아랍어 모델이라 맞지 않는다.
