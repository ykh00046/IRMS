...
def _iter_data_rows(ws):
    for row_idx, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        if not row:
            continue
        if row[COL_DATE] is None and row[COL_EMP_ID] is None:
            continue
        yield row


def _row_to_record(row: tuple[Any, ...]) -> dict[str, Any]:
    emp_id = _cell_str(row[COL_EMP_ID])
    if not emp_id:
        return {}
    return {
        "emp_id": emp_id,
        "name": _cell_str(row[COL_NAME]),
        "department": _cell_str(row[COL_DEPARTMENT]),
        "factory": _cell_str(row[COL_FACTORY]),
        "shift_time": _cell_str(row[COL_SHIFT_TIME]),
        "shift_group": _cell_str(row[COL_SHIFT_GROUP]),
        "job_type": _cell_str(row[COL_JOB_TYPE]),
        "gender": _cell_str(row[COL_GENDER]),
        "row": AttendanceRow(
            date=_cell_str(row[COL_DATE]),
            weekday=_cell_str(row[COL_WEEKDAY]),
            day_type=_cell_str(row[COL_DAY_TYPE]),
            check_in=_cell_time(row[COL_CHECK_IN]),
            check_out=_cell_time(row[COL_CHECK_OUT]),
            next_day=bool(_cell_float(row[COL_NEXT_DAY])),
            weekday_early=_cell_float(row[COL_WD_EARLY]),
            weekday_normal=_cell_float(row[COL_WD_NORMAL]),
            weekday_overtime=_cell_float(row[COL_WD_OVERTIME]),
            weekday_night=_cell_float(row[COL_WD_NIGHT]),
            holiday_early=_cell_float(row[COL_HD_EARLY]),
            holiday_normal=_cell_float(row[COL_HD_NORMAL]),
            holiday_overtime=_cell_float(row[COL_HD_OVERTIME]),
            holiday_night=_cell_float(row[COL_HD_NIGHT]),
            late_hours=_cell_float(row[COL_LATE]),
            early_leave_hours=_cell_float(row[COL_EARLY_LEAVE]),
            outing_hours=_cell_float(row[COL_OUTING]),
            note=_cell_str(row[COL_NOTE]),
        ),
    }
...
def employee_exists_in_any_month(emp_id: str) -> bool:
    ...
    for ym in available_months():
        ...
        try:
            ws = wb["Sheet1"] if "Sheet1" in wb.sheetnames else wb.active
            for row in _iter_data_rows(ws):
                if _cell_str(row[COL_EMP_ID]) == emp_id:
                    return True
        finally:
            wb.close()
    return False
