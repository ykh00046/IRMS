...
def ensure_account(emp_id: str) -> dict[str, Any]:
    """이미 프로비저닝된 계정을 반환하고 자동 생성은 하지 않는다."""
    record = _fetch(emp_id)
    if record is None:
        # Design Ref: attendance-public-api-hardening §7 — accounts must be
        # provisioned by a manager with a random temporary password.
        raise AttendanceAuthError(
...
def authenticate(emp_id: str, password: str) -> dict[str, Any]:
    ...
    if record is None:
        ...
        if not employee_exists:
            ...
            _log_failed_login(emp_id, "account_not_provisioned")
        # Plan SC-1/SC-2: never create a predictable credential during login,
        # and do not reveal whether an employee or account exists.
        raise AttendanceAuthError(
            code="INVALID_CREDENTIALS", status_code=status.HTTP_401_UNAUTHORIZED
        )
