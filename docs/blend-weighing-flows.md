...
> 규칙 옆에 `파일:함수`를 표기했다. 값 단위는 전부 `g` 고정. 계량값 **표시** 소수 자릿수는
> 레시피 허용 편차를 따른다(아래 "계량값 표시 정밀도"). 계산·검증·저장은 저울 해상도 기준 2자리 유지.
>
> 관련 소스
...
### 계량값 표시 정밀도 (2026-07-24)
- **규칙**: 계량값을 화면에 **표시**하는 소수 자릿수 = 현재 레시피 `tolerance_g`의 소수 자릿수.
  `0.05`(기본/NULL)→2자리, `0.1`/`0.5`→1자리, `1` 이상 정수→0자리(`blend_lib.toleranceDecimals`).
  허용 편차가 큰 레시피는 저울 해상도(0.1g 등)나 대용량 배치 때문이라, 저울이 못 찍는 소수
  (예: `4775.72`)를 목표로 보여주는 게 부자연스러워 편차 단위에 맞춘다.
- **적용 범위(표시 전용)**: 이론량 셀·placeholder, DHR 카드 이론/실제 합계·순편차, 증량(제안·폐기·
  적용) 모달·토스트 총량·새 이론량·추가분, 행/셀 "목표·추가" 배지, 추가 계량 모달 남은/목표/현재,
  부족 모달 이론/실제/부족량, 연속 화면 이론 셀·placeholder·로트 총량 배지. 두 화면 공통으로
  `blend.js`/`blend_continuous.js`의 로컬 `dp()`(=`toleranceDecimals(state.toleranceG)`)를 `fmt` 2번째 인자로 넘긴다.
- **미적용(그대로)**: 계산·편차 판정·저장 payload·서버 이론 도출(항상 2자리), 작업자가 입력한 실제량
  input 값, `/status` 기록·공식 DHR 문서, 행/셀 편차(`varianceDisplay` 등 2자리 유지)·허용 편차 안내 문구(`±0.05g`).
  레시피 전환 시 `state.toleranceG`가 갱신되어 표시 정밀도도 새 레시피 기준으로 다시 렌더된다.
...
|---|---|---|
| 구조 | 자재 행 × 1로트 | 자재 행 × N로트(자재 열 우선), 실제량·자재 LOT 이 셀별 |
| 총량/서명/반응기/비고 | 배치 1건 | **전 로트 공통** |
| 자재 LOT | 자재 행당 1개 | **셀별**((자재 × 로트) 각 셀 개별 `cells[i][j].lot` — 로트마다 다른 원료 봉지 LOT 을 그대로 저장. body `lots[j][i].material_lot`, 서버는 detail 별 LOT 을 이미 지원해 무변경) |
