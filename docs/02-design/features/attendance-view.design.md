...
| Auth | 별도 세션(`irms_att_session`) + bcrypt + 5분 비활성 타임아웃 + 5회 잠금 |
...
기존 `src/auth.py`의 bcrypt 래퍼 재사용:
```python
...
**첫 로그인 흐름:**
1. emp_id 엑셀에 존재 + attendance_users에 없음 → 비번 해시=bcrypt(emp_id) 로 자동 INSERT, `password_reset_required=1`
2. 입력 비번 검증
3. 세션에 `att_user` 저장
4. `password_reset_required=true` 반환 → 프론트가 비번 변경 페이지로 이동
...
1. 초기 상태 → 사번 171013 + 비번 171013 로그인 → 비번 변경 강제 페이지
2. 비번 변경(예: `newpass1`) → 조회 페이지
...
7. 관리자 비번 초기화 → 해당 사용자 다음 로그인 시 비번 변경 강제
