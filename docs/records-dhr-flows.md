...
- ✅ **비고 영역(표 아래 end_row+2 병합 셀) append**(양식 레이아웃 불변): 일괄 재생성 기록이면
  `(일괄 재생성 기록)` 표식을, 증량(rescale) 이력이 있으면 `rescale_summary_line`
  (`증량 N회: 1000→1050 (승인: 홍길동); 1050→1100 (부재: 사유)`, GAP-5) 요약을 이어 붙인다
  (둘 다면 한 줄에 함께). `record["is_bulk_regenerated"]` 플래그로 표식 여부 결정.
