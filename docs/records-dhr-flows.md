...
- **감사(규제 보존 before-image)**: `blend_record_update`(`blend_routes.py`) — 변경을 적용하기 **전에**
  현재 헤더(수정 가능 필드)와 상세 행을 포착해 details 에
  `{"before": {"header": {...}, "rows": [[자재명, 실제량, LOT, 이월(0/1), 수동(0/1)], ...]},
  "after_summary": {변경된 필드만 신규 값, 상세 변경 시 rows 포함}}` 로 남긴다. 스키마 변경 없이
  `audit_logs.details_json` 이 담으며, 상세 행은 terse 배열로 압축한다(전량 교체라 원본 복원 근거 보존).
...
- **hard 삭제 실체**: `record_delete_service.delete_blend_record`(`record_delete_service.py`) —
  `viscosity_readings.blend_record_id` 를 NULL 로 끊고, `blend_details`·`blend_records` 행 삭제,
  마지막에 `dhr_cache.purge` 로 잔류 PDF 캐시까지 정리(§7 POLISH-7a).
...
- **스캔효과**(`apply_scan_effects`, `dhr_pdf.py:190`): GaussianBlur + 노이즈 + 대비 + 밝기 + 종이톤(미색).
  파라미터는 서명 설정에서 옴.
- 일괄: `GET /blend/records/dhr-batch?ids=...`(`blend_dhr_batch`) → `build_batch_dhr_pdf`
  (기록당 1장, 최대 200건). `?sign=1` 지원. 여러 건을 **한 PDF 파일로 병합**(인쇄 지향).
- ZIP: `GET /blend/records/dhr-zip?ids=...`(`blend_dhr_zip`) → 각 기록의 PDF 를 단건 `/pdf` 와
  **동일 경로**로 생성(비서명본 캐시 재사용, `?sign=1` 서명 합성)한 뒤 **반제품명(product_name) 폴더**
  아래 `{product_lot}.pdf` 로 담아 zip 으로 내려준다(최대 200건). 폴더/파일명은 Windows 금지문자
  (`\ / : * ? " < > |`)를 `_` 로 치환(빈 폴더명→`기타`), 같은 폴더 내 LOT 중복은 `_2`,`_3`… 접미.
  한글은 UTF-8 플래그로 기록돼 Windows 압축 풀기에서 안 깨진다. 존재하지 않는 id 는 전체를
  실패시키지 않고 `누락.txt` 로 모아 남긴다. 파일명 `dhr-{yyyymmdd}.zip` + RFC5987 UTF-8 이름.
...
| **선택 출력(병합)** | `GET /blend/records/dhr-batch?ids=1,2,3`(PDF 한 파일) | 화면에서 고른 여러 건을 한 PDF 로(인쇄) |
| **선택 출력(ZIP)** | `GET /blend/records/dhr-zip?ids=1,2,3`(zip) | 반제품명 폴더별 `{LOT}.pdf` — 보관·배포 |
...
| `dhr_exported` | DHR 출력·다운로드(단건 Excel/PDF·일괄 PDF·ZIP·전체 Excel) | 사용자(무로그인=None) | product_lot / "N건" |
...
- ✅ **DHR 출력 감사(GAP-4 해소)**: `blend_export`(xlsx)·`blend_pdf`(pdf/pdf_signed)·`blend_dhr_batch`(pdf_batch)·
  `blend_dhr_zip`(zip/zip_signed)·`blend_export_all`(xlsx_all) 모두 `_audit_dhr_export` 로 `dhr_exported` 를 남긴다. details 에
...
### POLISH-7 — ✅ 해소 — 취소 기록 DHR 표식(7b) + hard 삭제 시 캐시 정리(7a)
- **7a(캐시 잔류)**: `record_delete_service.delete_blend_record`(`record_delete_service.py`)가 이제
  hard 삭제 시 `dhr_cache.purge(record_id)` 로 `data/dhr_cache/blend_{id}.pdf`·`.marker` 를 함께
  지운다(디스크 litter 제거). 캐시 미생성이어도 안전한 no-op. 테스트:
