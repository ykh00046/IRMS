...
- **삭제**: `DELETE /viscosity/readings/{id}`(`viscosity_routes.py:255-278`). 존재하지 않으면
  404, 성공 시 감사 `viscosity_reading_delete`. 화면에서는 관리자에게만 '삭제' 버튼이
  보인다(`viscosity.js:461-471`, `isManager`). **다만 라우트 자체는 무인증**(6절·GAP 참고).
...
- `GET /viscosity/products/{id}/export`(`viscosity_routes.py:280-316`), 관리자 UI 버튼
  (`viscosity.html:25`, `exportCsv` `viscosity.js:770-773`).
...
### GAP-1 (보안/권한) — manager 전용 점도 변경이 서버에서 강제되지 않음
`viscosity_mgr_router`(제품 생성·수정, 측정 삭제, CSV export)는 `api.py`에서 **아무 인증
의존성 없이** include된다(`src/routers/api.py:38, 59-60`). 각 라우트도
`get_current_user(request, required=False)`로 로그인 없으면 actor를 "현장"으로 기록할 뿐 거부하지
않는다(`viscosity_routes.py:158, 209, 261`). 모듈 docstring도 "op_router/mgr_router 둘 다
무인증 — 코드 구조 유지를 위해 두 라우터로 분리만 유지"라고 인정
(`viscosity_routes.py:5`, `48-52`). 화면은 `can_manage`로 설정/삭제 버튼만 숨긴다
(`viscosity.html:23`, `viscosity.js:28`). 결과: URL과 CSRF 토큰만 있으면 비관리자도
제품 삭제·수정·측정 삭제·CSV 추출이 가능. 점도 도메인 전체를 공용 단말용으로 의도적으로
공개한 정책의 연장이긴 하나, "관리자 전용"이라는 UI 표시와 실제 서버 강제 사이에 괴리가 있다.
정책상 허용이면 문서로 못박고, 아니면 `mgr_router`에 manager 의존성을 걸어야 한다.

### GAP-2 (판정 일관성) — CSV export의 status는 항상 전체 연도 기준
`viscosity_export`는 `analyze_product(connection, product)`를 year/reactor 없이 호출한다
(`viscosity_routes.py:286`). 따라서 CSV의 `status` 열은 **전 연도를 합친 표본**의 중심선·σ로
판정된다. 화면은 연도별(기본 최신 연도) 기준으로 판정하므로(`viscosity_service.py:474-537`,
`overview`가 최신 연도로 설정 → JS `state.year`), **같은 측정이 화면에서는 정상인데 CSV에서는
경고/이상으로(또는 반대로) 나올 수 있다.** 제품별로 연도마다 점도 대역이 다르다는 이 도메인의
전제(`viscosity_service.py:3-10`, `540-545`)와 정면으로 어긋난다. export가 현재 필터(연도/반응기)를
반영하지 않는 점도 함께 확인 필요.

### GAP-3 (경보 과민 잔존) — anomaly_spike는 일/주 완화에서 제외되지 않음
2026-07-22 완화는 `mean_shift`만 `coarse`(월/분기/연) 게이트에 넣었고
(`viscosity_service.py:427, 439`), `anomaly_spike`는 granularity 제한이 없다
(`432-438`). 일/주 단위에서 한 버킷에 이상 2건 이상이 몰리고 직전보다 늘면 여전히 경보가 뜬다.
spec을 좁게 잡은 반제품에서 일 단위 조회 시 재발 가능. 실제 과민 정도는 낮지만(하루 2건 이상
이상 필요), mean_shift만 손본 완화의 사각지대로 기록.
...
### POLISH-2 — sigma_k ≤ 2일 때 경고(warn) 구간 소멸
`WARN_SIGMA = 2.0`이 고정인데(`viscosity_service.py:27`) 제품 `sigma_k`는 1~6까지 허용된다
(`models.py:88, 106`). k ≤ 2로 설정하면 UCL(kσ)이 UWL(2σ)보다 안쪽에 놓여, 2σ 초과 값이
먼저 `value > ucl` anomaly로 걸리고 warn은 사실상 발생하지 않는다(`_classify`가 anomaly를
먼저 반환, `viscosity_service.py:281-297`). k를 3 이상으로 쓰는 운영 전제에선 무해하나,
설정 UI(`viscosity.html:201-202`)가 min=1을 허용하므로 경고 구간이 조용히 사라질 수 있다.
