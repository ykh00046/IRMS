...
## 10. 통계 제외는 어디까지 미치나 (2026-09-21)

현장 지적: APB 2026-08-10(LOT APB26081001, 439.5, 사유 '폐기')은 **통계 제외**인데도 PB
연계 산점도에 찍히고 추세선·상관 문장까지 끌고 갔다(기울기 -0.26 → -0.09, r -0.029 →
-0.009). 제외 규칙이 한 곳에만 적용돼 있었다. 규칙을 세 갈래로 못 박는다.

### 10.1 규칙

| 갈래 | 뜻 | 제외 측정은 |
|---|---|---|
| **통계** | 평균·σ·관리 한계·판정·이상 목록·추세 경보·기간 집계·상관/추세선·구간표·요약 카드의 '최근값' | 절대 들어가지 않는다 |
| **기록** | 사람이 *한 건의 측정*을 읽는 자리 — 측정 표·기록 조회 상세·PB LOT 역방향 조회·PB 연계 표 | 남는다. 항상 제외 표식과 사유를 달고 |
| **등록수** | "이 LOT 을 재기는 했나" — 등록 대기열·배합 기록의 점도 상태·트레이 알림의 pending | 그대로 '쟀다'로 센다 |

판단이 갈릴 때는 **그 숫자가 다른 측정과 섞여 하나의 값이 되는가**를 본다. 섞이면 통계,
한 건으로 읽히면 기록이다.

### 10.2 의도적인 예외 두 가지

- **배합 화면 LOT 경고**(`product_lot_alert`)는 제외된 측정도 보여 준다. 작업자에게는
...
`sourcePbLinkedReadings(readings)` 의 **기본값이 통계 표본**이다(제외 제거). 그림 아래
표만 `{ includeExcluded: true }` 로 다시 불러 제외 행을 제외 표식과 함께 싣는다. 패널
머리말은 그림에 실제로 들어간 건수를 말하고, 제외가 있으면 `제외 N건은 표에만` 을 덧붙인다.
`pbBandRows` 와 추세선 적합은 넘겨받은 목록에서 한 번 더 제외를 거른다(이중 방어).

### 10.4 Excel

측정을 내보내는 두 시트(`측정 원본`, `전체 측정`)는 `통계 제외`·`제외 사유` 를 **전용
열**로 싣는다. 종전에는 전체 Excel 의 판정 칸이 빈칸이라(라벨 사전에 `excluded` 가 없었다)
제외된 값이 정상 기록과 섞인 채 나갔고, 그 파일로 평균을 다시 내면 제외가 무의미해졌다.
읽기 전용 조회 도구(`tools/blend_query.py`)는 처음부터 `제외` 열을 갖고 있었다.
