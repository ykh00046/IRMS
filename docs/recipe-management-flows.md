...
### GAP 3 — "현재 tip" 정의가 3곳에서 서로 다르게 구현됨 — ✅ 해결(2026-07-23)
버전 이력의 독자적 정의(`max(active, key=(created_at,id))`)를 제거하고 **`resolve_chain_tip`
단일 소스**로 일원화했다. 이제 tip 판정은 두 형태만 남고 둘 다 같은 규칙이다:
1. 배합 저장 귀결·버전 이력: `resolve_chain_tip`(subtree 활성 `MAX(id)`) —
   `recipe_history` 가 이제 `current_id = resolve_chain_tip(connection, root_id)` 로 계산
   (`recipe_operator_routes.py` recipe_history). 전부 취소/초안이라 활성 후보가 없으면 최신본
   폴백으로 "전부 취소면 최신" 표시를 보존.
2. 목록/조회: `SUPERSEDED_RECIPE_IDS_SQL`(활성 후손 있는 조상 숨김) — `resolve_chain_tip` 과
   같은 규칙의 집합 표현(선형 체인에선 비숨김=tip 하나).
종전엔 이력이 `created_at` 우선 정렬이라 백필/임포트로 created_at↔id 가 역전되면 배합 저장
귀결(id 기준)과 **is_current 표시가 불일치**했다. 회귀 방지 `test_recipe_management.py::
test_branched_chain_tip_consistent_across_list_and_history`(분기 선점 데이터에서 이력
current_id = 배합 귀결 = 목록에 보이는 tip 이 모두 일치).
...
### GAP 5 — 승계된 category/hint 가 허용값 검증을 우회 — ✅ 해결(2026-07-23)
register 가 승계된 부모 `category` 또는 마스터 `category_hint` 를 저장하기 전에
`ALLOWED_CATEGORIES={"약품","합성","잉크","용수"}`(모듈 상수, `PUT /category` 와 동일 집합)로
**클램프**한다(`recipe_import_routes.py` register 루프). 허용 집합 밖(예: "기타")이면
`recipes.category` 를 NULL(미분류)로 저장하고 `logger.warning` 으로 서버 로그를 남긴다 —
현황/패널 드롭다운에 없는 값이 들어가 편집으로 정상화하기 애매해지던 문제를 차단. 허용값은
그대로 승계. 회귀 방지 `test_recipe_management.py::
test_revision_clamps_non_allowed_inherited_category_to_null`.

### GAP 6 — 기준 자재(anchor) 승계 무음 소실 — ✅ 해결(2026-07-23)
수정 등록 시 부모 anchor 의 `material_id` 가 새 버전 자재 집합에 없어 승계가 풀리면(자재
삭제 후 재생성으로 id 가 바뀐 경우 등) 여전히 NULL 로 떨어뜨리되(올바른 동작), 이제 등록
응답의 `warnings` 에 **"기준 자재 승계가 해제되었습니다 …"** 항목(level 2)을 실어 운영자에게
알린다(`recipe_import_routes.py` register 루프의 anchor 승계 분기). 종전엔 `tolerance_g`/
`category` 와 달리 anchor 만 조용히 사라져 신호가 없었다. 회귀 방지 `test_recipe_management.py::
test_revision_anchor_drop_emits_warning`.
