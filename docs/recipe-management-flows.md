...
- 파서: `import_parser.py` `parse_import_text` → `get_header_config()` 내부의
  **자재 3단 판정**(`import_parser.py:293-383`):
  ...
  3. **unknown** — 어디에도 없음. `_similar_candidates()`(difflib, cutoff 0.75) 로 유사
     ...
     - `allow_unknown_materials=True` → 코드 없이 자동 등록(경고로 강등).
     - 그 외(마스터 존재 + allow=False) → **errors 에 추가 → 등록 차단**.
       (파싱은 계속하되 `errors` 가 비면 안 되므로 register 400.)
...
- **개정 체인 처리:** `UPDATE recipes SET revision_of = NULL WHERE revision_of = ?`
  (`record_delete_service.py:47`) — 삭제 대상의 **자식들을 루트로 승격**시킨 뒤 삭제.
  → 5절/9절(GAP 1)에서 지적: 체인 중간 삭제 시 이력이 끊긴다.
...
### BUG 1 — 다중 반제품 임포트 + 명시 품목코드 = 전 행 동일 코드
`recipe_import_routes.py:215-238, 294-295`. `explicit_product_code` 는 루프 **밖에서 한 번**
계산·충돌검사되고, `for parsed_row in parsed["parsed_rows"]` 루프의 **모든 행**이
`effective_product_code = explicit_product_code` 를 받는다. raw 폴백으로 값행 2개 이상(서로
다른 반제품)을 한 번에 임포트하면서 명시 코드를 넣으면, **서로 다른 반제품들이 같은
product_code 로 등록**된다. "코드 1개 = 반제품 1개" 불변식 위반이고, 충돌 검사는 DB 만 보고
같은 배치 내 형제 행은 못 본다. (단일 BOM 편집기 경로에선 값행 1개라 미발생.)

### BUG 2 / GAP — 레시피 삭제가 개정 체인을 끊는다(고아 개정)
`record_delete_service.py:47`. `UPDATE recipes SET revision_of = NULL WHERE revision_of = ?`
는 삭제 대상의 **직계 자식만** 루트로 승격시킨다. 체인 `v1→v2→v3` 에서 **v2 를 삭제**하면
v3.revision_of=NULL 이 되어 **v3 가 독립 루트**가 되고 v1 과의 이력 연결이 끊긴다(버전 이력·
가족 판정에서 분리). 손자(v4 등)는 v3 밑에 유지되지만 v1 과의 계보는 영구 소실. UI 는
경고 없이 삭제만 알린다. `revision_of` 에 FK/ON DELETE 가 없어(schema.py:58) 앱 로직에
전적으로 의존.

### GAP 1 — `allow_unknown_materials` 를 켜는 UI 경로가 없음(도달 불가 분기)
`static/js` 어디서도 `allow_unknown_materials` 를 전송하지 않는다(`previewImport`/
`importRecipes`/`importWithAnchor` 본문 확인). 기본 False. 따라서 `item_code_master` 가
적재된 운영 환경(master 모드)에서 진짜 신규 자재(마스터·기존·완료레시피 어디에도 없음)는
**항상 errors 로 차단**되고, `import_parser.py:349-358` 의 "마스터에 없는 품목(확인 후 등록)"
경로는 **UI 에서 도달 불가능**하다. 운영자는 임포트로는 신규 자재를 못 넣고, 먼저 품목코드
탭(`POST /materials`)이나 마스터에 등록해야 한다. 의도된 정책일 수 있으나, 파서의 정교한
override 분기가 사장돼 있으니 (a) 확인 체크박스를 노출하거나 (b) 죽은 분기를 명시적으로
문서화할 필요가 있다.

### GAP 2 — 동시 수정 등록 레이스(체인 분기)
두 책임자가 같은 v1 을 각각 불러와 동시에 수정 등록하면 v2a·v2b 가 모두
`revision_of=v1` 로 생성된다. 개정 등록에 낙관적 잠금·버전 검사가 없다
(`recipe_import_routes.py` 의 INSERT 는 조건 없음). 결과: v1 은 활성 후손이 둘이라
숨겨지고 v2a·v2b 가 **동시에 tip** 이 된다. `by-product`/현황은 둘 다 노출,
`resolve_chain_tip`(`ORDER BY id DESC`)은 높은 id 하나만 골라 **한쪽이 조용히 유효 tip 에서
밀려난다**. 사용자에게 충돌 신호 없음.
...
### GAP 4 — `stage1_recipe_id` 의 참조 무결성 부재
- FK/ON DELETE 없음(`migrations.py:514`, `ensure_column` 만). **1차 레시피를 삭제해도**
  `delete_recipe` 가 `stage1_recipe_id` 를 정리하지 않는다(`record_delete_service.py`) →
  2차의 `stage1_recipe_id` 가 **댕글링**. detail/list 의 `LEFT JOIN` 은 조용히
  `stage1_product_name=NULL`, 가족 묶음은 `byId.has()` 실패로 무음 해제.
- `PUT /stage1`(`recipe_manager_routes.py:345-360`)은 자기 자신·미존재만 막고, 대상이 실제
  1차(중간체)인지·이미 2차인지·**A↔B 상호 지정(2노드 순환)** 인지는 검증 안 함. 현황
  가족 묶음은 emitted 집합으로 무한루프는 없으나 의미상 무의미한 링크가 허용됨.
