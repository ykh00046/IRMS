...
1. **마스터 임포트 범위**: ~~원자재 117행만~~ → **리허설 결과로 수정: AS/AC/AH/AW prefix 200행**
   (원자재 117 + 소모품 83 — 운영 자재 20종이 '소모품' 대분류에 있었음. AA=상품 제외)
   + 반제품(code2~4) 전부.
...
## 7. 운영 스냅샷 매칭 리허설 결과 (2026-07-16, backups/irms_20260716_175458)

- **자재(활성 75)**: 원자재 117행 기준 27 매칭(36%) → **소모품 대분류에서 20종 추가 발견**
  (BYK-199, AIBN, GMA…) → 범위 확장 시 **47/75(63%)**. 모호 1(GMMA — 마스터에 2코드).
- **자재 매칭에 반제품 마스터 포함 필요**: PB·SBCT·NUVBF-1 등은 **반제품을 원료로 쓰는**
  배합의 자재 — B코드(code2~4)와 매칭해야 한다(예: 자재 PB → B0020).
- **전체 마스터에도 없음 ~20종**: 카본블랙·RAVEN·BLACK·WHITE·CS Pigment(안료류)·메탄올 등
  — ERP 미등록 또는 다른 표기. **사용자 확인 목록**으로 P2 보고서에 출력.
- 데이터 정리 대상: 자재 '비고'(오등록), `Dibutyltin dilaurate ↔ dialurate`(오타 중복) —
  유사 중복 의심 5쌍 검출(GMA↔GMMA 등 정당한 구분 포함, 수동 판정).
- **반제품(제품명 35종)**: 17 매칭(48%, PB→B0020 확인). 미매칭 18 중 다수가 유사 후보 보유
  (SBCT-1→BC2000 계열, N2-TOP→B0127 계열 등 변형명) — 수동 확정 대상.

## 8. 등록·수정·복사 화면 재설계 (P5 확장 — 2026-07-16 사용자 요청)

현행: jspreadsheet 시트에 [반제품명 | 자재1 | 자재2 | …] — **자재가 열로 옆으로 계속
넓어지는 구조**. 자재가 많으면 가로 스크롤·+열 반복으로 불편.

방향: **자재를 행(세로 목록)으로 쌓는 BOM 편집기**로 전환:
- 상단: 반제품명(마스터 자동완성 → product_code·분류 자동 표시) · 기준량 · 허용편차 · 기준자재
- 본문: 자재 행 목록 — [자재 검색(코드·이름 자동완성, 마스터 연동)] [배합량(g)] [코드 표시] [삭제]
