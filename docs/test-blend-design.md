...
2. 정식 `add_reading` 은 시험 배합 기록의 LOT 이면 400 으로 거부한다. 정식 점도 통계·관리한계·이상·추세·
   알림·대시 카드는 시험 값을 볼 일이 없으므로 측정값 단위의 시험 필터가 필요 없다(1차 구현이 정식 표에
   `is_test=1` 로 남긴 값은 기동 때 `migrate_v1_test_viscosity` 가 옮기고, 옮기지 못한 고아만 `_fetch_readings` 한 곳에서 뺀다).
3. 배합 기록 단위의 제외는 유지한다: 정식 등록 대기열과 `daily_reading_reminders`(트레이)는 `blend_records.is_test = 1` 을 뺀다.
4. 점도 화면 '시험 점도' 탭은 반제품 선택과 무관하다. 시험은 필요할 때만 점도를 재므로 표에는 **기록한 시험 점도만**
   싣는다(최근 측정 먼저: 측정일·id 내림차순, 서버 검색: 시험명·LOT·작업자). 열: 측정일·시험명·제품 LOT·기준 레시피·
   점도·메모·등록(이름·시각·정정 표시)·관리(책임자 화면). 새 기록은 [점도 기록] 창에서 아직 점도가 없는 최근 완료 시험
   LOT(약 20건, 검색 가능)을 골라 점도값·측정일(기본 오늘)·메모를 넣는다. 미입력 행을 늘어놓거나 첫 행을 자동으로 고르지 않는다.
   API: `GET /api/viscosity/test-records?state=recorded|unrecorded|all&q=`(기본 recorded, 잘못된 값 422, `total`·`counts`).
   기록 조회 상세에서 점도가 없는 (취소 아닌) 시험 기록은 `/viscosity?tab=test&lot=…` 링크로 그 LOT 이 골라진 창을 연다
   (이미 값이 있으면 표에서 강조, 완료된 시험 LOT 이 아니면 안내만). 이 탭에는 요약 줄·관리 기준 상자·판정·그래프·규격선이 없다.
...
8. 화면 배치: 탭 줄이 맨 위다(측정 등록·추세·분석·PB 연계 | 이상 관리·시험 점도). 반제품 조회 조건·요약 줄·경보는
   앞 세 반제품 탭 안에만 있고(한 벌을 공유), 구분선 뒤 두 탭에는 보이지 않는다. 이상 관리는 자기 필터를 쓴다.
