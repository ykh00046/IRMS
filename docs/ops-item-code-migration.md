...
내용: 약어 자재 코드 부여(CH→AC0029, EE→AC0047, NVP→AS0005 등 17종) + 불용 자재
비활성화(현행 배합에 쓰지 않는 색소류, '비고' 오등록, GLYCEROL·dialurate 중복,
단독 'GMMA'). 비활성화는 목록에서만 빠지고 기존 기록은 보존된다.
...
- 보고 출력에서 '부여/비활성' 대상들을 훑어 이상 없으면 --apply.
- 재실행 안전(이미 코드 있는 자재·이미 비활성인 자재는 건너뜀).
- **보류 3건**(PB-APB · C-HEMA · PVP (K30P))은 이 스크립트가 건드리지 않는다 —
  정체 확인 후 스크립트를 갱신해 재실행.
