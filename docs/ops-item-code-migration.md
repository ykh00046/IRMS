...
미매칭·모호의 상당수는 **2026-07-17 운영자 확정본**(아래 §4.5 스크립트)이 자동 처리한다.
보고서에서 확인할 것은 다음 두 가지다:

- [ ] **분류 충돌(레시피)**: 기존 레시피 분류가 마스터의 제품구분(hint)과 다른 경우.
      자동으로 덮지 않으므로(보고만), 운영자가 어느 쪽이 맞는지 판단. 리허설에서는 7건.
- [ ] **레시피 미매칭(변형명)**: SBCT-1, N2-TOP, APB17 같은 변형명은 유사 후보가 찍히지만
      자동 확정은 안 된 것들. 유사 후보를 보고 맞으면 수동으로 product_code 지정(추후).

> 자재 쪽 미매칭 목록은 §4.5 확정본이 대부분 처리하므로, 거기 없는 새 이름이 보일 때만
> 별도로 메모해 두면 된다.
...
### 4.5 수동 확정본 반영 (자동 매칭 apply 다음에)

자동 매칭이 못 푼 자재들(IRMS 는 약어, ERP 는 풀네임이라 이름이 안 맞던 것)은 운영자가
코드를 확정했고, 그 확정본이 `tools/apply_manual_item_codes.py` 에 고정되어 있다.
내용: 약어 자재 코드 부여(CH→AC0029, EE→AC0047, NVP→AS0005, C-HEMA→AS0001,
PVP (K30P)→AC0011 등 20종) + 실수 등록 삭제('GLYCEROL'·'dialurate' 오타 중복은
참조를 정본으로 돌려놓고 삭제, '비고'·단독 'GMMA' 오등록도 삭제) + 색소류 4종
비활성화(과거 이력이 실재하므로 삭제하지 않음).

```
.venv\Scripts\python.exe tools\apply_manual_item_codes.py            ← 보고만(확인)
.venv\Scripts\python.exe tools\apply_manual_item_codes.py --apply    ← 반영
```

- 보고 출력에서 '부여/삭제/비활성' 대상들을 훑어 이상 없으면 --apply.
- 재실행 안전(이미 코드 있는 자재는 건너뜀, 지운 건 다시 안 지움).
- '삭제보류'가 찍히면 그 자재를 아직 참조하는 레시피가 있다는 뜻 — 자동으로 비활성화로
  대체되므로 진행에는 지장 없고, 해당 레시피 정리 후 재실행하면 지워진다.
- **보류 1건**(PB-APB — 어떤 레시피가 쓰는지 확인 후 결정)은 이 스크립트가 건드리지 않는다.
