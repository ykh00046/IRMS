...
- 현재 버전: **1.1.7** (TTS 견고화 + 초기 동기화 시 5분 내 공지 재생)
...
1. 관리자가 배포한 `IRMS-Notice-Setup-1.1.7.exe` 더블클릭
...
- **음성 합성 오류 자동 복구**: 드물게 Windows 음성엔진이 멈추면 30초 안에 자동 재시작 — 다음 공지부터 정상 재생.
...
- TTS watchdog: 단일 메시지 처리가 30초를 넘기면 SAPI 엔진을 강제 정지하고 다음 메시지에서 재초기화
