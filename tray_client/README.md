...
| 근태 이상 확인 | 실제 이상자 목록을 즉시 조회해 팝업 표시 |
| 근태 이상 보기 | 기본 브라우저로 `/attendance` 페이지 열기 |
...
- **팝업이 안 떠요** → "근태 이상 확인"으로 실제 이상자 목록을 즉시 조회. 목록이 비어 있으면 팝업이 뜨지 않음.
...
4. 우클릭 → "근태 이상 확인"으로 실제 이상자 조회와 팝업 동작 확인
