...
IRMS 공지방(`notice`) 메시지를 현장 PC에서 자동으로 음성(TTS)으로 재생하는 Windows 작업표시줄 상주 프로그램입니다.
...
1. 관리자가 배포한 `IRMS-Notice-Setup-1.0.0.exe` 더블클릭
...
| 음소거 / 음소거 해제 | 이 PC만 조용히 하기 (다른 PC에는 영향 없음) |
