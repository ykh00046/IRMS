...
| 트레이 우클릭 메뉴 | 설명 |
|------------------|------|
| **알림 사용** (체크) | 근태·점도 알림 전체 on/off. **끄면 재부팅해도 꺼진 상태 유지** |
| **저울 연동 사용** (체크) | 저울 HTTP 브릿지 on/off. **저울 있는 PC에서만 켜기** (기본 꺼짐, 설정 유지) |
| 현장 알림 오늘만 끄기 / 켜기 | 오늘 하루만 알림 팝업을 안 보이게. **자정에 자동 복귀** |
| 근태 알림 바로 확인 | 실제 이상자 목록을 즉시 조회해 팝업 표시 |
| 점도 알림 바로 확인 | 오늘 점도가 밀린 반제품을 즉시 팝업 |
| 저울 다시 연결 | (저울 켜짐일 때) 연결 안 된 저울 재연결 시도 |
| 근태 확인 / 반제품 제조 관리 / 점도 등록 | 기본 브라우저로 해당 페이지 열기 |
| 부팅 시 자동 실행 (체크) | Windows 시작 시 자동 실행 |
